...
```
#### 4. 图片压缩  
webpack 打包时，会根据 webpack.config.js 中 `url-loader` 设置的 `limit` 大小对图片进行处理，小于 limit 的图片会转化成 base64 格式，其他的不做操作。对于较大的图片我们还可以勇士 `image-webpack-loader` 来压缩图片。  
```
// webpack.config.js 
...
`Tree-Shaking ` 基于 import/export 语法， 可以在编译过程中获悉哪些模块没有被真正使用，在打包的时候会将这些没用的代码删除掉。
举个例子：  
在某个页面中  
```
...
注意：要使用 `Tree-Shaking `，必须保证引用的模块都是 ES6 规范的,如果项目中使用了 Babel，这就比较麻烦了，因为 Babel的预置默认把模块转译成了 **CommonJS** 模块，我们可以设置 modules: false 来解决此问题。  
